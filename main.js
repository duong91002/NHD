...
function phantrang() {
    ...
    for(var index in arrspphantrang) {
        document.getElementById('content1').innerHTML += '<div class="SP" onclick="ShowIfSP(' +    arrspphantrang[index].id + ')"><div class="SPimg"><img src="' +arrspphantrang[index].img + '" alt="" /></div><div style="text-align: center;"><div class="TenSP">' +    arrspphantrang[index].Name + '</div><div class="GiaSP">' +    arrspphantrang[index].price + '</div></div></div>';
    }
...
function ShowIfSP(id) {
    ...
    for (var index in SanPham) {
        if (SanPham[index].id == id) {
            document.getElementById("ThongTinSP").innerHTML = '<div><img src="' + SanPham[index].img + '" alt=""><div id="ThongTinSP_text"><div id="name_TTSP">' + SanPham[index].Name + '</div><div id="gia_TTSP">' + SanPham[index].price + '</div><div ><input class="SoluongSP" type="button" value="-" onclick="TangSLSP(this.value)"><input class="SoluongSP" id="soluongSP" type="text" min="1" max="10" value="1"><input class="SoluongSP"type="button" value="+" onclick="TangSLSP(this.value)"></div><input id="order" type="button" onclick="check();getTTSP();" value="Order"></div> <div id="backsp" onclick="TatTTSP()" style="float: right;">BACK</div></div>'
        }
    }
}
function TatTTSP() {
...
function search() {
    ...
    var hang = document.getElementById('lochang').value;
    console.log(hang);
...
function phanhang() {
    ...
    for (var index in hang) {
        document.getElementById('lochang').innerHTML += '<option value="' + hang[index].idhang + '">' + hang[index].tenhang + '</option> ';
    }
}